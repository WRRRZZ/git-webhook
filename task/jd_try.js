...
function requireConfig() {
  return new Promise(resolve => {
    ...
    $.notify = $.isNode() ? require('../send_notify') : {sendNotify:async () => {}}
        ...
        if (err) {
          ...
          if (data.success && data.data) {
            for (let index = 0; index < data.data.data.length; index++) {
              ...
              if (element.tip && element.tip.indexOf('恭喜您获得试用资格') != -1){
                // console.log(`待领取：${element.trialName}`)
                $.waitTryList.push(element.trialName)
              } else {
                // console.log(`往期试用：${element.trialName}`)
                $.pastTryList.push(element.trialName)
              }
